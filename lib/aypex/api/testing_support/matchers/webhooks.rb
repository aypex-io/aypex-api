...
# `Aypex::Webhooks::Subscribers::QueueRequests.call` method
...
RSpec::Matchers.define :emit_webhook_event do |event_to_emit|
  match do |block|
    queue_requests = instance_double(Aypex::Webhooks::Subscribers::QueueRequests)
    ...
    expect(queue_requests).to(have_received(:call).with(event_name: event_to_emit, webhook_payload_body: webhook_payload_body.to_json).once)
  end
