...
shared_context 'Platform API v2' do
  ...
  let(:admin_app) { Aypex::OauthApplication.find_or_create_by!(name: 'Admin Panel', scopes: 'admin', redirect_uri: '') }
  let(:read_app) { Aypex::OauthApplication.find_or_create_by!(name: 'Read App', scopes: 'read', redirect_uri: '') }
  let(:oauth_token) do
