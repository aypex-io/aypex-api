...
describe Aypex::Webhooks do
  describe "#disable_webhooks" do
    ...
    after { ENV["DISABLE_AYPEX_WEBHOOKS"] = "true" }

    describe "the value of DISABLE_AYPEX_WEBHOOKS environment variable" do
      before { ENV["DISABLE_AYPEX_WEBHOOKS"] = "some_value" }
      ...
      describe "when an error is not raised" do
        it "sets it to the original value" do
          ...
          expect(ENV["DISABLE_AYPEX_WEBHOOKS"]).to eq("some_value")
        end
      end
    ...
    describe "when webhooks are already disabled" do
      before { ENV["DISABLE_AYPEX_WEBHOOKS"] = "true" }
