...
describe Aypex::Webhooks::Subscribers::MakeRequest do
  ...
  describe "#failed_request?" do
    ...
    describe "setting read_timeout with AYPEX_WEBHOOKS_TIMEOUT" do
      context "without AYPEX_WEBHOOKS_TIMEOUT" do
        before { ENV["AYPEX_WEBHOOKS_TIMEOUT"] = nil }
      ...
      context "with AYPEX_WEBHOOKS_TIMEOUT" do
        before do
          ENV["AYPEX_WEBHOOKS_TIMEOUT"] = aypex_webhooks_timeout.to_s
          allow(Net::HTTP).to receive(:new).with(uri.host, uri.port).and_return(http)
        end

        after { ENV["AYPEX_WEBHOOKS_TIMEOUT"] = nil }

        let(:aypex_webhooks_timeout) { 15 } # time in seconds

        it "sets Net::HTTP#read_timeout= to the integer value of AYPEX_WEBHOOKS_TIMEOUT" do
          expect(http).to receive(:read_timeout=).with(aypex_webhooks_timeout)
          subject
        end
      end
    end
