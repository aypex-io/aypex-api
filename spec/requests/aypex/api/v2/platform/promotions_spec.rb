...
describe "Promotion API v2 spec" do
  ...
  let(:new_promotion_attributes) do
    {
      ...
      promotion_rules_attributes: [
        {
          code: "ESJD",
          type: "Aypex::Promotion::Rules::User"
        }
      ],
      promotion_actions_attributes: [
        {
          type: "Aypex::Promotion::Actions::FreeShipping",
          position: 1
        }
      ]
    }
  ...
  describe "promotions#create" do
    context "with valid params" do
      ...
      it "creates and returns a promotion" do
        ...
        expect(json_response["included"][1]).to have_attribute(:type).with_value("Aypex::Promotion::Actions::FreeShipping")

        # included promotion_rule
        expect(json_response["included"]).to include(have_type("promotion_rule"))
        expect(json_response["included"][2]).to have_attribute(:type).with_value("Aypex::Promotion::Rules::User")
