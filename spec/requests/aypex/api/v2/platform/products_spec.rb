...
describe "API V2 Platform Products Spec" do
  ...
  describe "products#index" do
      ...
      context "sorting by price" do
        ...
        context "ascending order" do
          ...
          it "returns products sorted by price" do
            expect(json_response["data"].count).to eq store.products.count
            expect(json_response["data"].pluck(:id)).to eq store.products.joins(master: :prices).select("#{store.products.table_name}.*, #{Aypex::Price.table_name}.amount").distinct.order("#{Aypex::Price.table_name}.amount").map(&:id).map(&:to_s)
          end
        end
        ...
        context "descending order" do
          ...
          it "returns products sorted by price with descending order" do
            expect(json_response["data"].count).to eq store.products.count
            expect(json_response["data"].pluck(:id)).to eq store.products.joins(master: :prices).select("#{store.products.table_name}.*, #{Aypex::Price.table_name}.amount").distinct.order("#{Aypex::Price.table_name}.amount DESC").map(&:id).map(&:to_s)
          end
        end
      end
    ...
    context "fetch products by currency params" do
