...
describe 'API V2 Storefront Checkout Spec', type: :request do
    ...
    shared_examples 'perform next' do
      context 'without line items' do
        ...
        it 'cannot transition to address without a line item' do
          expect(json_response['error']).to include(I18n.t('aypex.there_are_no_items_for_this_order'))
        end
      end
  ...
  order_contexts.each do |order_context|
    describe "checkout#update (#{order_context})" do
      ...
      before do
        allow_any_instance_of(Aypex::PaymentMethod).to receive(:source_required?).and_return(false)
        allow_any_instance_of(Aypex::Order).to receive_messages(confirmation_required?: true)
        allow_any_instance_of(Aypex::Order).to receive_messages(payment_required?: true)
      end
      ...
      shared_examples 'perform update' do
        ...
        context 'shipment' do
          ...
          let(:new_selected_shipping_rate_id) { Aypex::ShippingRate.last.id }
        ...
        context 'payment' do
          ...
          context 'payment source' do
            ...
            it 'updates payment method with source' do
              expect(order.payments).not_to be_empty
              expect(order.payments.last.source.name).to eq('Aypex Commerce')
          ...
          context 'when the gateway rejects the payment source' do
            ...
            before do
              allow_any_instance_of(Aypex::Order).to receive(:update_from_params).and_raise(Aypex::Core::GatewayError.new('Card declined'))
              execute
            end
        ...
        context 'email' do
          ...
          it 'updates email' do
            expect(order.reload.email).to eq('guest@aypex.io')
          end

          it 'returns updated email' do
            expect(json_response['data']).to have_attribute(:email).with_value('guest@aypex.io')
          end
        end
    ...
    context 'for user with store credits' do
      ...
      shared_examples 'valid payload' do |amount|
        it 'returns StoreCredit payment' do
          ...
          payment = Aypex::Payment.find(json_response['data']['relationships']['payments']['data'][0]['id'])
          expect(payment.amount).to eq amount
          expect(payment.payment_method.class).to eq Aypex::PaymentMethod::StoreCredit
        end
      end
      ...
      context 'with option include' do
        let!(:payment) { Aypex::Payment.all.first }
  ...
  describe 'checkout#remove_store_credit' do
    ...
    before do
      create(:store_credit_payment_method, stores: [store])
      Aypex::Checkout::AddStoreCredit.call(order: order)
      execute
    end
  ...
  describe 'checkout#payment_methods' do
    ...
    shared_examples 'returns a list of available payment methods' do
      ...
      it 'returns valid payment methods JSON' do
        ...
        expect(json_response['data'][0]).to have_attribute(:settings).with_value(payment_method.settings.as_json)
      end
    end
  ...
  describe 'checkout#select_shipping_method' do
    ...
    let!(:shipping_method) do
      create(:shipping_method, zones: [zone], shipping_categories: [shipping_category]) do |shipping_method|
        shipping_method.calculator.amount = 10
        shipping_method.calculator.save
      end
    end
    let!(:shipping_method_2) do
      create(:shipping_method, zones: [zone], shipping_categories: [shipping_category]) do |shipping_method|
        shipping_method.calculator.amount = 15
        shipping_method.calculator.save
      end
    end
    let!(:shipping_method_3) do
      create(:shipping_method, zones: [create(:zone)], shipping_categories: [shipping_category]) do |shipping_method|
        shipping_method.calculator.amount = 5
        shipping_method.calculator.save
      end
    end
    let(:shipment) { order.shipments.first }
