  ...
  describe 'account#show' do
    ...
    context 'with params "include=default_billing_address"' do
      ...
      it 'returns account data with included default billing address' do
        ...
        expect(json_response['included'][0]).to eq(Aypex::Api::V2::Storefront::AddressSerializer.new(user.billing_address).as_json['data'])
      end
    end
    ...
    context 'with params "include=default_shipping_address"' do
      ...
      it 'returns account data with included default shipping address' do
        expect(json_response['included']).to    include(have_type('address'))
        expect(json_response['included'][0]).to eq(Aypex::Api::V2::Storefront::AddressSerializer.new(user.shipping_address).as_json['data'])
      end
    end
    ...
    context 'with params include=default_billing_address,default_shipping_address' do
      ...
      it 'returns account data with included default billing and shipping addresses' do
        expect(json_response['included']).to    include(have_type('address'))
        expect(json_response['included'][0]).to eq(Aypex::Api::V2::Storefront::AddressSerializer.new(user.billing_address).as_json['data'])
        expect(json_response['included'][1]).to eq(Aypex::Api::V2::Storefront::AddressSerializer.new(user.shipping_address).as_json['data'])
      end
    end
  ...
  describe 'users#create' do
    ...
    context 'valid request' do
      ...
      it 'creates and returns user' do
        created_user = Aypex::Config.user_class.last
    ...
    context 'invalid request' do
      ...
      before do
        allow(Aypex::Api::Dependencies).to receive_message_chain(:storefront_account_create_service, :constantize).and_return(service)
      end
      ...
      describe 'mocks' do
        ...
        it { expect(Aypex::Api::Dependencies).to receive_message_chain(:storefront_account_create_service, :constantize).and_return(service) }
  ...
  describe 'users#update' do
    ...
    context 'invalid request' do
      ...
      context 'password mismatch error' do
        ...
        before do
          allow(Aypex::Api::Dependencies).to receive_message_chain(:storefront_account_update_service, :constantize).and_return(service)
        end
        ...
        describe 'mocks' do
          ...
          it { expect(Aypex::Api::Dependencies).to receive_message_chain(:storefront_account_update_service, :constantize).and_return(service) }
