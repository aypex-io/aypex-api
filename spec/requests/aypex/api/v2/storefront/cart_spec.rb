...
describe "API V2 Storefront Cart Spec" do
  ...
  shared_context "coupon codes" do
    ...
    let!(:promotion_action) { Aypex::PromotionAction.create(promotion_id: promotion.id, type: "Aypex::Promotion::Actions::FreeShipping") }
  end

  describe "cart#create" do
    let(:order) { Aypex::Order.last }
    let(:params) { {} }
      ...
      context "user association" do
        ...
        context "with canceled order" do
          ...
          it "returns a valid cart JSON response with new order" do
            new_order = Aypex::Order.find(json_response["data"]["id"])
          ...
          it "creates the new order without line items" do
            expect(Aypex::Order.find(json_response["data"]["id"]).line_items.count).to eq(0)
          end
        end
      end
    end
  ...
  describe "cart#add_item" do
    ...
    before do
      Aypex::PermittedAttributes.line_item_attributes << :cost_price
    end
  ...
  describe "cart#destroy" do
    ...
    shared_examples "destroying order" do
      it "destroys the order" do
        expect { execute }.to change { Aypex::Order.count }.by(-1)
      end
    end
  ...
  describe "cart#apply_coupon_code" do
    ...
    shared_examples "apply coupon code" do
      ...
      context "with coupon code for free shipping" do
        ...
        let(:adjustment_value_in_money) { Aypex::Money.new(adjustment_value, currency: order.currency) }
    ...
    shared_examples "remove coupon code" do
      context "with coupon code applied" do
        before do
          ...
          Aypex::PromotionHandler::Coupon.new(order).apply
          order.save!
        end
        ...
        context "removes coupon code correctly" do
          ...
          it "does not include the promotion in the response" do
            expect(json_response["included"]).to match_array([])
          end
        end
      ...
      context "when multiple coupon codes are applied" do
        ...
        it "has applied promotions" do
          expect(order.promotions).to include(promotion_with_item_adjustment)
        end

        context "removes coupon code correctly" do
          let!(:coupon_code) { promotion_with_item_adjustment.code }
          before { execute }
