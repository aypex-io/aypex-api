...
RSpec.describe Aypex::Api::V2::Storefront::WishlistsController do
  ...
  describe "#default" do
    context "no wishlist" do
      it "must create a new default wishlist" do
        expect { get "/api/v2/storefront/wishlists/default", headers: headers_bearer }.to change { Aypex::Wishlist.count }.from(0).to(2)
      end
    end
    ...
    context "has default wishlist" do
      ...
      it "must return the existing default wishlist" do
        expect { get "/api/v2/storefront/wishlists/default", headers: headers_bearer }.not_to change { Aypex::Wishlist.count }
      end
    end
    ...
    context "has default wishlist in another store" do
      ...
      it "creates a new default wishlist for the current store" do
        expect { get "/api/v2/storefront/wishlists/default", headers: headers_bearer }.to change { Aypex::Wishlist.count }.from(1).to(2)
      end
    end
  end
  ...
  describe "#add_item" do
    ...
    context "when a variant is already in the wishlist" do
      ...
      it "return the existing wished_item - quantity attribute omitted" do
        post "/api/v2/storefront/wishlists/#{user.wishlists.first.token}/add_item", headers: headers_bearer,
          params: {
            variant_id: set_variant.id.to_s
          }
