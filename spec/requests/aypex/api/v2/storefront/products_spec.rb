...
describe "API V2 Storefront Products Spec" do
  ...
  before { Aypex::Api::Config.api_v2_per_page_limit = 4 }
      ...
      context "sorting by price" do
        context "ascending order" do
          ...
          it "returns products sorted by price" do
            expect(json_response["data"].count).to eq store.products.available.count
            expect(json_response["data"].pluck(:id)).to eq store.products.available.joins(master: :prices).select("#{store.products.table_name}.*, #{Aypex::Price.table_name}.amount").distinct.order("#{Aypex::Price.table_name}.amount").map(&:id).map(&:to_s)
          end
        end
        ...
        context "descending order" do
          ...
          it "returns products sorted by price with descending order" do
            expect(json_response["data"].count).to eq store.products.available.count
            expect(json_response["data"].pluck(:id)).to eq store.products.available.joins(master: :prices).select("#{store.products.table_name}.*, #{Aypex::Price.table_name}.amount").distinct.order("#{Aypex::Price.table_name}.amount DESC").map(&:id).map(&:to_s)
          end
        end
      end
          ...
          context "when updated_at date is different for each product" do
            shared_examples "returns products in ascending updated_at order" do
              it "returns products in ascending updated_at order" do
                expect(json_response["data"].count).to eq(store.products.available.count)
                expect(json_response["data"].map { |p| p["attributes"]["sku"] }).to eq(available_products_with_master.sort_by(&:updated_at).map(&:sku))
              end
            end
