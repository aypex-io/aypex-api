...
class DummyController < Aypex::Api::V2::BaseController
  private
...
describe Aypex::Api::V2::BaseController, type: :controller do
  ...
  describe '#error_during_processing' do
    ...
    before do
      @routes = ActionDispatch::Routing::RouteSet.new.tap do |r|
        r.draw { get 'index', to: 'aypex/api/v2/base#index' }
      end
    end
    ...
    shared_examples 'rescues from error' do
      it do
        ...
        expect(subject).to receive(:aypex_current_user).at_least(:once).and_return(user)
        expect_next_instance_of(::Aypex::Api::ErrorHandler) do |instance|
  ...
  describe '#serializer_params' do
    ...
    let(:user) { nil }
    let(:price_options) { { tax_zone: Aypex::Zone.new } }
    ...
    shared_examples 'returns proper values' do
      before do
        ...
        allow(dummy_controller).to receive(:aypex_current_user).and_return(user)
