...
describe Aypex::Webhooks::HasWebhooks do
  ...
  context 'with DISABLE_AYPEX_WEBHOOKS equals "true" (set in spec_helper)' do
    let(:queue_requests) { instance_double(Aypex::Webhooks::Subscribers::QueueRequests) }

    before do
      allow(Aypex::Webhooks::Subscribers::QueueRequests).to receive(:new).and_return(queue_requests)
  ...
  context 'without DISABLE_AYPEX_WEBHOOKS' do
    ...
    context 'with a class name with multiple words' do
      let(:webhook_payload_body) do
        Aypex::Api::V2::Platform::CmsPageSerializer.new(
          cms_page,
          include: Aypex::Api::V2::Platform::CmsPageSerializer.relationships_to_serialize.keys
          ).serializable_hash
      end
      ...
      context 'when using touch without arguments' do
        it do
          expect do
            ...
            Aypex::Product.find(product.id).touch
          end.not_to emit_webhook_event(event_name)
        end
      end
    ...
    context 'on touch events from callbacks' do
      ...
      let(:body) do
        Aypex::Api::V2::Platform::StoreSerializer.new(
          store2,
          include: Aypex::Api::V2::Platform::StoreSerializer.relationships_to_serialize.keys
        ).serializable_hash
      end
  ...
  describe '.default_webhook_events' do
    it 'return the default events' do
      expect(Aypex::Order.default_webhook_events).to eq(%w[order.create order.delete order.update])
    end
  end
  ...
  describe '.supported_webhook_events' do
    context 'when there are custom supported events' do
      it 'returns the default and custom events' do
        ...
        expect(Aypex::Order.supported_webhook_events).to contain_exactly(*expected_webhook_events)
      end
    end

    context 'when there are no custom supported events' do
      it 'return the default events' do
        expect(Aypex::Zone.supported_webhook_events).to eq(Aypex::Zone.default_webhook_events)
      end
    end
  end
end
