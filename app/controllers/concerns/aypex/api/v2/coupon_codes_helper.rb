    ...
    module V2
      module CouponCodesHelper
        ...
        def check_coupon_codes
          aypex_current_order.promotions.coupons.map(&:code)
        end

        def select_error(coupon_codes)
          result = coupon_handler.new(aypex_current_order).remove(coupon_codes.first)
          result.error
        end
        ...
        def select_errors(coupon_codes)
          ...
          coupon_codes.each do |coupon_code|
            results << coupon_handler.new(aypex_current_order).remove(coupon_code)
          end
