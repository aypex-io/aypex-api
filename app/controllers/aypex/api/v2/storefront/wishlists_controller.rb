module Aypex
  module Api
    module V2
      module Storefront
        class WishlistsController < ::Aypex::Api::V2::ResourceController
          before_action :require_aypex_current_user, except: [:show]
          before_action :ensure_valid_quantity, only: [:add_item, :set_item_quantity]

          def show
            aypex_authorize! :show, resource
            super
          end

          def create
            aypex_authorize! :create, Aypex::Wishlist

            @wishlist = aypex_current_user.wishlists.new(wishlist_attributes)
          ...
          def destroy
            ...
            if resource.destroy
              ...
              render_error_payload(I18n.t("aypex.api.the_wishlist_could_not_be_destroyed"))
            end
          end
          ...
          def default
            ...
            @default_wishlist = aypex_current_user.default_wishlist_for_store(current_store)

            render_serialized_payload { serialize_resource(@default_wishlist) }
          end

          def add_item
            aypex_authorize! :create, Aypex::WishedItem
            ...
            if resource.wished_items.present? && resource.wished_items.detect { |wv| wv.variant_id.to_s == params[:variant_id].to_s }.present?
              ...
              @wished_item = Aypex::WishedItem.new(params.permit(:quantity, :variant_id))
          ...
          def set_item_quantity
            aypex_authorize! :update, wished_item
          ...
          def remove_item
            aypex_authorize! :destroy, wished_item
          ...
          def resource_serializer
            ::Aypex::Api::V2::Storefront::WishlistSerializer
          end
          ...
          def serialize_wished_item(wished_item)
            ::Aypex::Api::V2::Storefront::WishedItemSerializer.new(
              wished_item,
