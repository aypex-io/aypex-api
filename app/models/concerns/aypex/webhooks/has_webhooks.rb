    ...
    module HasWebhooks
      ...
      included do
        ...
        def queue_webhooks_requests!(event_name)
          return if disable_aypex_webhooks?
          return if Aypex::Webhooks::Subscriber.active.with_urls_for(event_name).none?
          return if update_event?(event_name) && updating_only_ignored_attributes?
          return if webhook_payload_body.blank?

          Aypex::Webhooks::Subscribers::QueueRequests.call(
      ...
      def resource_serializer
        @resource_serializer ||=
          begin
            ...
            "Aypex::Api::V2::Platform::#{demodulized_class_name}Serializer".constantize
          end
      end
